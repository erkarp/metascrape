module.exports = {
  ...
  {
    return ( !href || href === '/' || href[0] === '#' || href.includes('mailto') );
  },
  ...
  {
    return ( !url.includes('.') || url.includes('./') || this.haveValidFile(url) );
  },

  haveValidFile: function(path)
  {
    return ( !path.includes('.') || path.includes('.htm') || path.includes('.php') || path.includes('.xml') || path.includes('.asp') );
  }
